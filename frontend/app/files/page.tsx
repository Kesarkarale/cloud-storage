...
export default function FilesPage() {
  ...
  async function shareFileWithUser(
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/shares?${params.toString()}`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <Modal onClose={onClose}>
      ...
      <button
        ...
        className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl border border-blue-200 bg-blue-50 py-3 text-sm font-semibold text-blue-700 transition hover:bg-blue-100 disabled:opacity-60 dark:border-blue-500/20 dark:bg-blue-500/5 dark:text-blue-300 dark:hover:bg-blue-500/10"
      >
    ...
    <Modal onClose={() => {
      ...
      <form onSubmit={handleSubmit} className="mt-6">
        ...
        <input
          ...
          autoFocus
          disabled={sharing}
          className="mt-2 h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-blue-400 focus:ring-4 focus:ring-blue-500/10 dark:border-white/10 dark:bg-white/5 dark:text-white"
        />
        ...
        <div className="mt-2 grid grid-cols-2 gap-2">
          ...
          >
            ...
            <span className="mt-1 block text-[11px] opacity-75">Can view the file</span>
          </button>
          ...
          >
            ...
            <span className="mt-1 block text-[11px] opacity-75">Can edit the file</span>
          </button>
        </div>

        <p className="mt-3 text-xs leading-5 text-slate-400">
          The person must already have an account in this Cloud Storage application.
        </p>
        ...
        <div className="mt-6 flex justify-end gap-2">
          <button
            ...
            onClick={onClose}
            disabled={sharing}
            className="rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-600 transition hover:bg-slate-50 disabled:opacity-50 dark:border-white/10 dark:text-slate-300 dark:hover:bg-white/5"
          >
            Cancel
          </button>
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-blue-600/20 transition hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {sharing && (
              <Loader2 className="h-4 w-4 animate-spin" />
            )}
